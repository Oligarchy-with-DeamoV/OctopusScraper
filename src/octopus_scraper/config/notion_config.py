...
from .models import NotionDatabaseConfig, ScraperConfig
...
class NotionConfigClient:
    ...
    async def load_scrapers_config(self) -> List[ScraperConfig]:
        ...
        try:
            logger.info("Loading scrapers configuration from Notion", 
            ...
            for record in response["results"]:
                try:
                    ...
                    logger.debug("Loaded scraper config", 
                               scraper_name=scraper.name,
                               fetcher=scraper.fetcher)
                except Exception as e:
                    logger.error("Failed to parse scraper record", 
                               record_id=record["id"],
                               error=str(e))
                    # Continue with other scrapers
                    continue
            
            logger.info("Successfully loaded scrapers configuration", 
            ...
            logger.error("Failed to load scrapers configuration", 
                        error=str(e), 
                        exc_info=True)
            raise
    ...
    async def check_config_changes(self) -> bool:
        ...
        try:
            ...
            logger.debug("Configuration change check completed", 
            ...
            logger.error("Failed to check configuration changes", 
                        error=str(e))
