...
class ConfigManager:
    ...
    def __init__(self, notion_config: NotionDatabaseConfig, service_config: ServiceConfig):
    ...
    async def load_initial_config(self) -> List[ScraperConfig]:
        ...
        try:
            ...
            logger.error("Failed to load initial configuration", error=str(e), exc_info=True)
            raise
    ...
    async def reload_config_if_changed(self) -> bool:
        ...
        try:
            ...
            current_config_hash = self._current_version.config_hash if self._current_version else ""
            ...
            change_summary = self._create_change_summary(old_version, self._current_version)
    ...
    def validate_scrapers_config(self, scrapers: List[ScraperConfig]) -> List[str]:
        ...
        for scraper in scrapers:
            ...
            if scraper.fetcher not in ["rsshub", "direct_rss"]:
                errors.append(f"Scraper '{scraper.name}' has invalid fetcher: {scraper.fetcher}")
            
            if scraper.priority < 1 or scraper.priority > 10:
                errors.append(f"Scraper '{scraper.name}' has invalid priority: {scraper.priority}")
    ...
    def get_current_config_status(self) -> ConfigStatus:
        ...
        next_check = None
        if self._last_check:
            next_check = self._last_check + timedelta(seconds=self.service_config.config_refresh_interval)
    ...
    def _create_change_summary(self, old_version: Optional[ConfigVersion], 
                             new_version: ConfigVersion) -> str:
        ...
        if old_count == new_count:
            ...
            return f"Removed {old_count - new_count} scrapers ({old_count} → {new_count})"
    ...
    def get_status(self):
        ...
        return ConfigStatus(
            ...
            last_check=self._last_check or datetime.now(),
            next_check=(self._last_check or datetime.now()) + timedelta(seconds=self.service_config.config_refresh_interval),
            is_healthy=self._is_healthy,
            error_message=self._error_message,
        )
