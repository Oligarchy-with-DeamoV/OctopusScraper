...
def test_octopus_with_html_and_llm_processors(octopus_config, structured_llm_processor_config):
    ...
    # 限制抓取数量
    modified_config["scrapers_config_with_fetch_params"][0]["fetch_params"] = {"limit": 2}

    print(f"配置的Scraper数量: {len(modified_config['scrapers_config_with_fetch_params'])}")
    print(f"配置的处理器: {list(modified_config['scrapers_config_with_fetch_params'][0]['scraper_config'].content_processor_configs.keys())}")
    ...
    for i, content in enumerate(processed_contents):
        ...
        # 检查内容特征
        markdown_indicators = ["#", "**", "*", "[", "](", "\n\n"]
        has_markdown = any(indicator in content.content for indicator in markdown_indicators)
        if has_markdown:
...
@pytest.mark.integrate_test  
def test_scraper_with_html_and_llm_processors_standalone(structured_llm_processor_config):
    ...
    # 显示HTML处理后的内容片段
    print(f"\n📖 HTML处理后的内容片段:")
    print(content.content[:400] + "..." if len(content.content) > 400 else content.content)
