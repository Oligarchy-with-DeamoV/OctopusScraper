...
class TestLLMProcessorIntegration:
    ...
    def test_llm_processor_basic_functionality(self, llm_processor_config, sample_content):
        """测试LLM处理器的基本功能"""
        # 创建LLM处理器实例
    ...
    def test_llm_processor_with_structured_output(self, structured_llm_processor_config, sample_content):
        """测试带结构化输出的LLM处理器"""
        # 创建带结构化输出的LLM处理器
        ...
        if result_contents:
            ...
            try:
                ...
                print(f"结构化输出: {json.dumps(parsed_summary, ensure_ascii=False, indent=2)}")
            except json.JSONDecodeError:
                pytest.fail("LLM输出不是有效的JSON格式")

    def test_llm_processor_multiple_contents(self, llm_processor_config, sample_contents):
        """测试LLM处理器处理多个内容"""
        processor = LLMProcessor(llm_processor_config)
