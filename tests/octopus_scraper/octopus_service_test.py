...
class TestServiceLifecycle:
    ...
    async def test_reload_octopus_config(self, mock_app, mock_config_manager):
        ...
        mock_app.ctx.octopus._notion_api_config = {"api_key": "test", "database_id": "test"}
        
        mock_config_manager.get_current_scrapers.return_value = []
        mock_config_manager.get_current_version.return_value = Mock(version_id="test_v2")
...
class TestConfigEndpoints:
    ...
    async def test_get_config_status_success(self):
        ...
        mock_status.version = Mock(
            version_id="test_v1",
            timestamp=Mock(),
            config_hash="test_hash",
            scrapers_count=1
        )
        mock_status.version.timestamp.isoformat.return_value = "2025-01-01T00:00:00"
        mock_status.scrapers = []
    ...
    async def test_refresh_config_success(self):
        ...
        with patch('octopus_scraper.octopus_service.app') as mock_app, \
             patch('octopus_scraper.octopus_service.reload_octopus_config') as mock_reload:
    ...
    async def test_validate_config_success(self):
        ...
        with patch('octopus_scraper.octopus_service.app') as mock_app:
            mock_manager = Mock()
            mock_manager.notion_client.load_scrapers_config = AsyncMock(return_value=mock_scrapers)
